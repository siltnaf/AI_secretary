...
bool ESP_Brookesia_StatusBar::setWifiIconState(int state) const
{
    if (state == 0) {
        ...
        if (station != nullptr && esp_netif_get_ip_info(station, &ip_info) == ESP_OK && ip_info.ip.addr != 0) {
            wifi_ap_record_t access_point = {};
            state = esp_wifi_sta_get_ap_info(&access_point) == ESP_OK
                    ? (access_point.rssi >= -60 ? 3 : (access_point.rssi >= -75 ? 2 : 1))
                    : 2;
        }
    }
...
{
    return setWifiIconState(static_cast<int>(state));
}
